...
`.trim() + '\n'
  ...
  function suggestAnswerFromQuestion(q: string): string {
    ...
    if (s.includes('cable') && s.includes('diameter')) return 'Cable diameter is 5 mm.'
    if (s.includes('slot') && (s.includes('count') || s.includes('how many'))) return 'Use 4 slots.'
    if (s.includes('spacing')) return 'Set slot spacing to 10 mm.'
    if (s.includes('mount')) return 'Use adhesive mount.'
    if (s.includes('desk') && s.includes('thickness')) return 'Desk thickness is 20 mm.'
    if (s.includes('height')) return 'Height should be 20 mm.'
    if (s.includes('width')) return 'Width should be 30 mm.'
    if (s.includes('length') || s.includes('depth')) return 'Length should be 50 mm.'
    ...
    >
      ...
      <div className="flex-1 space-y-6">
        ...
        >
          ...
          {!loading && (questions.length > 0 || assumptions.length > 0) && (
            <div className="flex flex-wrap gap-2 mt-2">
              {questions.map((q, idx) => (
                <button
                  key={`q-${idx}`}
                  type="button"
                  onClick={() => setUserPrompt(suggestAnswerFromQuestion(q))}
                  ...
                  title="Prefill an answer based on this question"
                >
                  {q}
                </button>
              ))}
